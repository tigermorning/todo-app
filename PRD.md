...
## 19. 사용법 문서 보강 + 한국어 사용법 추가 (2026-07-21)

사용자가 "사용법에 이 앱 사용방법을 상세히 넣어줘야 해"라고 요청. 두 가지로 나눠 처리하기로 확인함: (1) 기존 영어 README의 "How to use it" 보강, (2) 한국어 사용법 문서 신규 작성.

- **README.md 보강**: 기존엔 Features 목록에만 있던 다중 선택 설명을 "How to use it" 안에도 단계별로 옮기고, 그동안 문서화가 빠져 있던 **카테고리 관리**(추가/이름·아이콘·그룹 수정/삭제/할 일 하나만 바꾸기)와 **검색·필터**(검색창+카테고리 드롭다운+날짜 필터가 함께 적용되는 방식) 섹션을 새로 추가함.
- **`사용법.md`(신규)**: 영어 README의 직역이 아니라, "서버가 이미 떠 있다"고 가정하고 실제 사용자 관점으로 새로 구성한 한국어 안내서. 실행 방법(§1)부터 빠른 입력/상세 입력/카테고리/검색·필터/다중 선택/사이드바(달력+Tracker 3종)/지난 할 일 확인/구글 캘린더 연동(다중 캘린더·일괄 수락 포함)/알려진 한계까지 10개 섹션으로 구성.
- 이번 질문에서 함께 나온 "모바일에서 쓸 수 있냐 / 앱스토어에 팔 수 있냐"는 별개 주제로 답변만 하고 실제 작업(예: `--host 0.0.0.0`으로 LAN 접속 설정)은 사용자가 "일단 보류"를 선택해 진행하지 않음 — 나중에 요청 시 처리.
- README.md 상단 안내문과 "Project structure" 트리, "How to use it" 진입부에 `사용법.md` 링크를 추가해 서로 찾아가기 쉽게 함.
